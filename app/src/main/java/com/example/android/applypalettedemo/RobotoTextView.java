...
public class RobotoTextView extends TextView {
    ...
    void extractFont(AttributeSet attrs) {
        ...
        TypedArray typedArray =
                getContext()
                        .getTheme()
                        .obtainStyledAttributes(attrs, R.styleable.Roboto, 0, 0);

        int robotoFontIndex =
                typedArray.getInteger(R.styleable.Roboto_robotoFont, -1);

        typedArray.recycle();

        String[] resourceStringArray =
                getResources().getStringArray(R.array.roboto_font_file_names);

        if (robotoFontIndex < 0 || robotoFontIndex >= resourceStringArray.length) {
            return;
        }

        String robotoFont = resourceStringArray[robotoFontIndex];
